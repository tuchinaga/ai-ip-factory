...
- 単語画面で、データ読み込み前にカテゴリタブや単語一覧が空のまま表示され、読み込み完了時にレイアウトが崩れる(ポップインする)問題を修正。読み込み中はDashboard等と同じくローディング表示に統一し、データが揃ってから一括で表示するように変更

## v1.5.0 (2026-08-08)

- レスポンシブ対応の総点検を実施
  - **[重要]** Library一覧カードの削除ボタン、Seed詳細の画像操作(メインに設定/削除)がホバー時のみ表示だったため、タッチ操作のみのスマホでは実質使えなかった問題を修正。スマホでは常設の操作ボタンを表示し、デスクトップでは従来通りホバーで表示する形に変更
  - iOS Safariで入力欄タップ時に自動ズームしてしまう問題を修正(フォントサイズをモバイルで16px以上に調整)
  - 単語画面のカテゴリ追加フォーム(key/表示名の2つの入力欄)がスマホ幅で窮屈だったのを縦積みに変更
  - 単語画面の「AIで単語を提案」ヘッダーが折り返し時に崩れないよう調整
  - Seed詳細の画像グリッドをスマホでは2列、それ以外は3列に調整
